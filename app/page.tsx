...
export default function HackathonPageClient() {
  // ✅ Type-safe params for Next.js 15 / Vercel builds
  const params = useParams() as Record<string, string>
  const id = params?.id ?? ''

  const [hackathon, setHackathon] = useState<Hackathon | null>(null)
  const [loading, setLoading] = useState(true)
  const [ideasRaw, setIdeasRaw] = useState<string>('')
  ...
  const [lastProposalTime, setLastProposalTime] = useState(0)
  ...
  return (
    <main className="max-w-3xl mx-auto p-6">
      ...
      <section className="mb-6">
        ...
        {proposalOutput && (
          <div className="p-3 bg-gray-50 rounded text-sm prose">
